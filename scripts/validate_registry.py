...
def validate_registry(registry_dir: str | Path, *, verbose: bool = False) -> ValidationResult:
    """Validate a registry directory.

    Returns a ValidationResult that includes schema errors and invariant violations.
    The registry can be either:
      - registry_*.json (canonical), or
      - cnfa_tag_registry_canonical_*.yml/.yaml (legacy YAML subset).
    """
    reg_dir = Path(registry_dir)
    data, reg_path = load_registry(reg_dir)

    # Expected structure: {"tags": {<tag_id>: {...}, ...}, ...}
    tags: dict[str, dict] = {}
    if isinstance(data, dict):
        maybe_tags = data.get("tags", {})
    ...
    for tag_id, tag in tags.items():
        if not isinstance(tag, dict):
            result.add_error(tag_id, "_tag", f"Tag entry must be a mapping/dict, got {type(tag).__name__}")
            continue
    ...
    if verbose:
        ...
        print(f"Loaded registry: {reg_path} (tags={len(tags)})")
